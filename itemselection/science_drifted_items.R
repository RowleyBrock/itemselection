...
estimates %>% 
  select(grade, remove) %>% 
  unnest()%>%
 write_csv ("science_drifted_items.csv")

dir_create("plots")
plot_titles <- paste0("plots/g", estimates$grade, "science_ipdrift.pdf")
